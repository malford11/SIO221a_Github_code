function [data] = Class10_Ben(ChunkSize)
...
data.dt =mean(diff(data.time),'omitnan'); %(30/60)/24; %convert dt=30 min into CPD
N=size(data.Chunks);

[data.spectra,data.f_vector,data.df] = Spectra2(data.Chunks,data.dt,'none');
%calculate spectra
[data.p,data.variance,data.sum_spec] = Parseval(data.data,data.spectra,data.df);
